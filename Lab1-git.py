...
#%%
## 1.1.1 Data exploration outliers and heavy hitters
# Create scatterplot over transferred amounts over time(stamps)
fig = plt.figure()
ax = plt.gca()
ax.scatter(df['timestamp'], df['amount'], c='blue')
fig.suptitle ('Amounts over time(stamps)', fontsize = 20)
plt.xlabel('Timestamp')
plt.ylabel('Amount')
ax.set_ylim(0, max('amount'))

# Create another plot that displays transferred amounts over time(stamps)
# lm = sns.lmplot(data=df, y = "amount", x = "timestamp", truncate=False)

# Create a histogram of frequency of transactions over time(stamps)
ax = df.plot.bar('timestamp')
ax.set_xlabel("Timestamp")
ax.set_ylabel("Number of transactions")
props = {
        'title': 'Transaction frequency per timestamp' }
ax.set(**props)
fig = plt.gcf()
